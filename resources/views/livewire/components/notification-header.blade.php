<li class="header-notification">
    <a aria-controls="notificationcanvasRight" 
       ...
       role="button">
        ...
        @if($unreadCount > 0)
            <span class="position-absolute translate-middle p-1 bg-success border border-light rounded-circle animate__animated animate__fadeIn animate__infinite animate__slower">
            ...
            <div class="head-container notification-head-container">
                        ...
                        <div class="message-content-box flex-grow-1 ps-2">
                            ...
                            <div class="notification-details">
                                <span class="d-inline-block f-w-500 me-1">
                                    Réf: <span class="text-primary">{{ $notification->data['order_number'] ?? 'N/A' }}</span>
                                </span>
                                @if(isset($notification->data['customer_name']))
                                    |
                                    <span class="d-inline-block f-w-500 ms-1">
                                        Client: <span class="text-primary">{{ $notification->data['customer_name'] }}</span>
                                    </span>
                                @endif
                ...
                @empty
                    <div class="hidden-massage py-4 px-3 text-center">
                        <img alt="No notifications" 
                             class="w-50 h-50 mb-3 mt-2"
                             src="{{ asset('assets/images/icons/bell.png') }}">
                        <div>
                            <h6 class="mb-0">Aucune notification</h6>
                            <p class="text-muted">Vous n'avez pas de nouvelles notifications concernant les commandes.</p>
                        </div>
                    </div>
                @endforelse
            </div>
