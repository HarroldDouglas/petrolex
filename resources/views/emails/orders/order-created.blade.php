...
@section('title', __('email.order_created_subject', ['order_number' => $order->order_number]))

@section('header-title', __('email.order_created_subject', ['order_number' => $order->order_number]))
...
@section('content')
    ...
    <p>{{ __('email.order_created_message', [
    ...
    <div style="background-color: #e8f5e8; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #28a745;">
        ...
        @endif
        <p><strong>{{ __('email.order_status') }}:</strong> <span style="color: #28a745;">{{ $order->status->label ?? ucfirst($order->status) }}</span></p>
    </div>

    <p>{{ __('email.order_created_footer_message') }}</p>

    <div style="background-color: #fff3cd; padding: 15px; border-radius: 6px; margin: 15px 0; border-left: 4px solid #ffc107;">
        <p style="margin: 0; color: #856404;"><strong>{{ __('email.note') }}:</strong> {{ __('email.order_created_note') }}</p>
    </div>
@endsection
