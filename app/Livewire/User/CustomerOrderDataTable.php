...
class CustomerOrderDataTable extends BaseDataTable
{
    ...
    {
        return 'commandes_client_'.$this->user->id;
    }

    public function builder(): Builder
    {
        return Order::query()
            ->where('customer_id', $this->user->customer->id)
        ...
        return [
            ...
            Column::make('Livreur', 'delivery_person_id')
                ...
                ->format(function ($value, $row) {
                    ...
                    return new HtmlString(
                        '<a href="'.route('users.delivery.details', $user->id).'" class="btn-link fw-bold">'.trim(($user->first_name ?? '').' '.($user->last_name ?? '')).'</a>'
                    );
                }),
