...
class VerifyPaymentStatusJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            // Determine if this should be retried based on status AND error type
            ...
            if (! $response->success) {
                ...
                // Use the network timeout flag from result calculation
                ...
                if ($isNetworkTimeout && $this->attemptCount < self::MAX_RETRY_ATTEMPTS) {
                    ...
                    // Regular pending status retry (original logic)
            ...
            if ($result['should_retry'] ?? false) {
                ...
                // Use service to find OrderPayment instead of direct database query
            ...
            // Check if this is a network timeout exception that should be retried
            ...
            if ($isNetworkTimeout && $this->attemptCount < self::MAX_RETRY_ATTEMPTS) {
                ...
                // Mark payment as failed if this was the final attempt
                if ($isNetworkTimeout) {
                    $mockResponse = (object)[
                        'success' => false,
